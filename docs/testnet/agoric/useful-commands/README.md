...
```bash
agd tx slashing unjail --from wallet --chain-id agoric-emerynet-5 --gas auto --gas-adjustment 1.4 -y
```
...
```bash
agd tx distribution withdraw-all-rewards --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Withdraw commission and rewards from your validator

```bash
agd tx distribution withdraw-rewards $(agd keys show wallet --bech val -a) --commission --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Delegate tokens to yourself

```bash
agd tx staking delegate $(agd keys show wallet --bech val -a) 1000000ubld --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Delegate tokens to validator

```bash
agd tx staking delegate <TO_VALOPER_ADDRESS> 1000000ubld --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

Redelegate tokens to another validator

```bash
agd tx staking redelegate $(agd keys show wallet --bech val -a) <TO_VALOPER_ADDRESS> 1000000ubld --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

Unbond tokens from your validator

```bash
agd tx staking unbond $(agd keys show wallet --bech val -a) 1000000ubld --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```
...
```bash
agd tx gov vote 1 yes --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Vote 'No'

```bash
agd tx gov vote 1 no --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Vote 'Abstain'

```bash
agd tx gov vote 1 abstain --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```

#### Vote 'NoWithVeto'

```bash
agd tx gov vote 1 nowithveto --from wallet --chain-id agoric-emerynet-5 --gas-adjustment 1.4 --gas auto -y
```
...
sed -i.bak -e "s%^address = \"tcp://0.0.0.0:1317\"%address = \"tcp://0.0.0.0:${CUSTOM_PORT}317\"%; s%^address = \":8080\"%address = \":${CUSTOM_PORT}080\"%; s%^address = \"0.0.0.0:9090\"%address = \"0.0.0.0:${CUSTOM_PORT}090\"%; s%^address = \"0.0.0.0:9091\"%address = \"0.0.0.0:${CUSTOM_PORT}091\"%" $HOME/.agoric/config/app.toml
```
...
```bash
sed -i 's|^indexer *=.*|indexer = "null"|' $HOME/.agoric/config/config.toml
```

Enable indexer

```bash
sed -i 's|^indexer *=.*|indexer = "kv"|' $HOME/.agoric/config/config.toml
```
